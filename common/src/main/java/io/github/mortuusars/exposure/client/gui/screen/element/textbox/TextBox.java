...
public class TextBox extends AbstractWidget {
        ...
        if (isHovered && visible && isActive() && event.button() == 0) {
            ...
            if (index >= 0) {
                if (index == lastIndex && currentTime - lastClickTime < 250L) {
                    ...
                    textFieldHelper.setCursorPos(index, event.hasShiftDown());
                }
                clearDisplayCache();
            }
    ...
    protected void changeLine(int yChange, KeyEvent event) {
        int cursorPos = this.textFieldHelper.getCursorPos();
        int line = this.getDisplayCache().changeLine(cursorPos, yChange);
        this.textFieldHelper.setCursorPos(line, event.hasShiftDown());
    }
    ...
    protected void keyHome(KeyEvent event) {
        if (event.hasControlDown()) {
            ...
            this.textFieldHelper.setCursorPos(lineStartIndex, event.hasShiftDown());
        }
    }
    ...
    protected void keyEnd(KeyEvent event) {
        if (event.hasControlDown()) {
            ...
            this.textFieldHelper.setCursorPos(lineEndIndex, event.hasShiftDown());
        }
    }
}
