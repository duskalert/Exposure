...
public record FramePredicate(Optional<ExposureIdentifier> identifier,
    ...
    public boolean matches(Frame frame) {
        return (identifier.isEmpty() || identifier.get().equals(frame.identifier()))
                ...
                && (frame.extraData().get(Frame.FOCAL_LENGTH).map(focalLength::matches).orElse(false))
                && (frame.extraData().get(Frame.LIGHT_LEVEL).map(lightLevel::matches).orElse(false))
                && (frame.extraData().get(Frame.DAY_TIME).map(dayTime::matches).orElse(false))
